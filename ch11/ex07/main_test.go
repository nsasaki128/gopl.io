...
func TestAdd(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			for i := 0; i < 1024; i++ {
				if x.Has(i) != y.Has(i) {
					t.Errorf("Error in case %s, x.Has(%d)=%t and y[%d]=%t, these are expected as same.", testCase.name, i, x.Has(i), i, y.Has(i))
				}
			}
...
// This benchmark test is used for ch11 ex07
...
func BenchmarkMapIntSet_Add(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var mapIntSet MapIntSet
		rng := rand.New(rand.NewSource(seed))
		for j := 0; j < 512; j++ {
			mapIntSet.Add(rng.Intn(intSize))
		}
	}
}
...
func BenchmarkIntSet_UnionWith(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		rng := rand.New(rand.NewSource(seed))
		for j := 0; j < 512; j++ {
			setA.Add(rng.Intn(intSize))
			setB.Add(rng.Intn(intSize))
		}
		setA.UnionWith(&setB)
	}
}
func BenchmarkMapIntSet_UnionWith(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var mapIntSetA MapIntSet
		var mapIntSetB MapIntSet
		rng := rand.New(rand.NewSource(seed))
		for j := 0; j < 512; j++ {
			mapIntSetA.Add(rng.Intn(intSize))
			mapIntSetB.Add(rng.Intn(intSize))
		}
		mapIntSetA.UnionWith(&mapIntSetB)
	}
}
